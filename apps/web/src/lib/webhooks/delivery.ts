...
/**
 * Queues and immediately attempts delivery of a webhook event to all registered
 * endpoints for the merchant that subscribe to this event type.
 * Failed deliveries are queued for retry by the billing engine's webhook retry job.
 */
...
async function deliverToEndpoint(
  endpoint: { id: string; endpointId: string; url: string; secret: string },
...
/** Exponential backoff: 5m, 30m, 2h, 5h, 10h (max 5 retries) */
