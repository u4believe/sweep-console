...
export type SettleBridge = (mintTxHash: Hex) => Promise<void>;

/// Fetch the burn's attestation and mint on Arc, then hand off to `settle`.
/// Returns the mint tx hash, or null when Iris isn't ready yet — in which case the
/// row stays "burned" and the next pass resumes it. Only a real mint failure throws.
async function mintAndSettle(bridge: BridgeTransfer, settle: SettleBridge): Promise<Hex | null> {
...
export async function advanceBridge(bridge: BridgeTransfer, settle: SettleBridge): Promise<Hex | null> {
  let b = bridge;
  if (b.status === "pulled") {
