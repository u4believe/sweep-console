...
const SOURCE_CHAINS: Record<string, { id: number; rpc: string }> = {
  "Base Sepolia": { id: 84532, rpc: process.env.BASE_SEPOLIA_RPC_URL ?? "https://sepolia.base.org" },
  "Arbitrum Sepolia": { id: 421614, rpc: process.env.ARBITRUM_SEPOLIA_RPC_URL ?? "https://sepolia-rollup.arbitrum.io/rpc" },
  "OP Sepolia": { id: 11155420, rpc: process.env.OPTIMISM_SEPOLIA_RPC_URL ?? "https://sepolia.optimism.io" },
};
...
async function cmdFund(to: string | undefined, send: boolean, amountEth: string) {
  ...
  console.log(`from ${funder.address}\nto   ${to}\neach ${amountEth} ETH${send ? "" : "   (dry run — pass --send)"}\n`);

  for (const [name, { id, rpc }] of Object.entries(SOURCE_CHAINS)) {
...
async function main() {
  ...
  const amount = amountIdx >= 0 ? rest[amountIdx + 1] ?? "0.02" : "0.02";

  if (cmd === "new") cmdNew(arg);
  else if (cmd === "fund") await cmdFund(arg, rest.includes("--send"), amount);
  else { console.error("Commands: new <file> | fund <address> [--send] [--amount 0.02]"); process.exit(1); }
}
